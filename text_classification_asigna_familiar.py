...
datos = datos[((datos['flg_cat']==97) & (datos['target']==0))==False]
pd.crosstab(datos['flg_cat'],datos['target'])
datos['target'] = np.where(datos['target']==1,1,0)    
...
#-------------------------------------------------------------------*
print(result)
count_class_0, count_class_1 = df_temp.target.value_counts().sort_index()

df_class_0 = df_temp[df_temp['target'] == 0]
df_class_1 = df_temp[df_temp['target'] == 1]

df_class_0_sample = df_class_0.sample(count_class_1,replace=True)
...
dir_os_tf_idf = gir_repo_dir+'/feat_engine_asigna_fam_os_tf_idf.pickle'
...
dir_os_tfidf_rf = gir_repo_dir+'/asigna_fam_os_tf_idf_rf.joblib'
