...
datos['target'] = np.where((datos['OBSERVACIONES_A'].str.contains(r'(?i)(rot|raja|negro|quiña|malogr|rob|extrav|p.rdi)|((\bno\b).*func)', regex=True)) | 
                           ...
                           datos['target'])
#-----------------------------------------------------------------------------*
datos['target'] = np.where((datos['OBSERVACIONES_A'].str.contains(r'(?i)(\bsin\b|falta|\bno\b).*((chic|chip|internet|cobert|conect|saldo|megas)|(plan|datos))', regex=True)) & (datos['target']==0),
